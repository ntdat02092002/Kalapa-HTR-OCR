...
from .utils import StringDistance, extract_digit, remove_accents
...
class AddressCorrection:
    ...
    def __init__(self, cost_dict_path=None, provinces_path=None, districts_path=None, wards_path=None, underwards_path=None):
        ...
        with open(wards_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                districts = districts.split('|')
                wards = wards.split('|')
                if len(wards) == 2 and not wards[0].startswith('phường'):
                    wards = [wards[0]]
                provinces = provinces.split('|')
                for province in provinces:
        ...
        with open(underwards_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                underwards = underward.split('|')
                underward = underwards[0]
    ...
    def correct_prefix_number(self, phrase, correct_phrases, nb_candidates=2, distance_threshold=40):
        candidates = [(None, distance_threshold)] * nb_candidates
        max_diff_length = distance_threshold
        for correct_phrase in correct_phrases:
            if abs(len(phrase) - len(correct_phrase)) >= max_diff_length:
                continue
            if remove_accents(phrase) != remove_accents(correct_phrase):
                continue
            if extract_digit(correct_phrase) != extract_digit(phrase):
                distance = 100
            else:
    ...
    def address_correction(self, address, correct_th=30):
        '''
        ...
        prefix_number = ('số', 'đội', 'xóm', 'khu', 'ngách', 'đường', 'tổ', 'ngõ', 'phường', 'thôn', 'ấp', 'khối', 'kp', 'quận', 'phố')
        for i in range(1, min(5, len(tokens))):
            if not tokens[i].isalpha() and 'p' not in tokens[i]:
                corrected_token = self.correct_prefix_number(tokens[i-1], prefix_number, nb_candidates=1, distance_threshold=15)[0]
